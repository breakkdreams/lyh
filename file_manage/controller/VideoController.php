...
class VideoController extends PluginBaseController
{
    public function getConfig(){
        $pfm=new PluginFileModel();
        $module_info = getModuleConfig('file_manage','config','video_config.json');
...
function upload_video($isModule=false)
{
    $config=$this->getConfig();
    if(!$_FILES){
        zy_array(false,"请上传视频","",-99,$isModule);
    }
    $files=$_FILES['file'];
    //print_r($files);
    // 判断错误号
    if (is_array(@$files['tmp_name'])) {
        zy_array(false,"暂不支持多视频上传","",-100,$isModule);
    ...
    if (@$files['error'] == 00) {
        ...
        if (!move_uploaded_file($files['tmp_name'], $destName)) {
            ...
            $res=$this->add_file($fileName,$destName,2);
    ...
    public function index()
    {
        ...
        return $this->fetch('/file/video_index');
    }

    /**
     * desription 添加到数据库
     */
    function add_file($filename,$path,$filetype,$des="视频"){
